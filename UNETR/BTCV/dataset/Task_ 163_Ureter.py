...
if __name__ == "__main__":
    ...
    base = "/data2/sukmin/_has_Task162_Ureter"

    task_id = 163
    task_name = "Ureter"
    foldername = "Task%03.0d_%s" % (task_id, task_name)

    data_path = '/data2/sukmin/monai_dataset'
    ...
    # 나중에 test inference를 위해 폴더는 만들어놓음
    for p in test_patients:
        curr = os.path.join(base, p)
        label_file = os.path.join(curr, "segmentation.nii.gz")
    ...
    json_dict['for_test'] = [{'image': "./imagesTr/%s.nii.gz" % i.split("/")[-1], "label": "./labelsTr/%s.nii.gz" % i.split("/")[-1]} for i in
                            test_patient_names]
